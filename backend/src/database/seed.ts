...
/**
 * Railway عادةً يمرر DATABASE_URL + (PGHOST/PGPORT/PGUSER/PGPASSWORD...)
 * والاتصال الخارجي غالباً يحتاج SSL.
 */
function buildDataSource() {
  ...
  // ✅ الأفضلية: DATABASE_URL (Railway)
  if (process.env.DATABASE_URL) {
    return new DataSource({
      type: 'postgres',
      url: process.env.DATABASE_URL,
      // في Railway غالباً لازم SSL
      ssl: isTruthy(process.env.DB_SSL) || process.env.NODE_ENV === 'production'
  ...
  // ✅ fallback: متغيرات مخصصة (local/dev)
  ...
  const database = process.env.DB_NAME || process.env.PGDATABASE || 'omcs';
...
async function seed() {
  ...
  // ⚠️ ملاحظة: هنا كلمات المرور فيها ! حسب كودك الأصلي
