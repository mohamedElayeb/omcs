...
async function seed() {
    const ds = new DataSource({
        ...
        entities: [Branch, User, Category, Product, ProductVariant, Inventory, StockMovement, StockTransfer, Sale, SaleItem, Return, ReturnItem, DailyClosing, PriceHistory],
        synchronize: true,
    });
    ...
    const b1 = await branchRepo.save(branchRepo.create({ name: 'السياحية', nameEn: 'Al-Siyahiya', address: 'Tripoli, Libya', phone: '+218-91-0000001' }));
    const b2 = await branchRepo.save(branchRepo.create({ name: 'النوفليين', nameEn: 'Al-Nawfaliyeen', address: 'Tripoli, Libya', phone: '+218-91-0000002' }));
    ...
    await userRepo.save([
        userRepo.create({ email: 'mohamed@outletmaster.ly', passwordHash: hash, fullName: 'Mohamed Elayeb', role: UserRole.OWNER, maxDiscountPercent: 100, maxDiscountValue: 999999 }),
        userRepo.create({ email: 'manager1@outletmaster.ly', passwordHash: hash, fullName: 'محمد الكيلاني', role: UserRole.MANAGER, branchId: b1.id, overridePin: '1234', maxDiscountPercent: 50, maxDiscountValue: 500 }),
        userRepo.create({ email: 'manager2@outletmaster.ly', passwordHash: hash, fullName: 'سالم العريبي', role: UserRole.MANAGER, branchId: b2.id, overridePin: '5678', maxDiscountPercent: 50, maxDiscountValue: 500 }),
        userRepo.create({ email: 'cashier1@outletmaster.ly', passwordHash: cashierHash, fullName: 'أحمد بن علي', role: UserRole.CASHIER, branchId: b1.id, maxDiscountPercent: 10, maxDiscountValue: 50 }),
        userRepo.create({ email: 'cashier2@outletmaster.ly', passwordHash: cashierHash, fullName: 'عمر الشريف', role: UserRole.CASHIER, branchId: b2.id, maxDiscountPercent: 10, maxDiscountValue: 50 }),
    ]);
    ...
    for (const p of products) {
        const product = await prodRepo.save(prodRepo.create({
            name: p.name, brand: p.brand, categoryId: cats[p.catIdx].id,
        }));

        for (const v of p.variants) {
            const margin = ((v.sale - v.cost) / v.sale) * 100;
            const variant = await varRepo.save(varRepo.create({
                sku: v.sku, productId: product.id, size: v.size, color: v.color,
                costPrice: v.cost, salePrice: v.sale, profitMargin: Math.round(margin * 100) / 100,
            }));
            ...
            for (const branch of [b1, b2]) {
                ...
                await invRepo.save(invRepo.create({
                    variantId: variant.id, branchId: branch.id, quantity: qty,
                    lastRestocked: new Date(),
                }));
                await movRepo.save(movRepo.create({
                    variantId: variant.id, branchId: branch.id,
                    action: StockMovementAction.RESTOCK,
                    quantityChange: qty, quantityAfter: qty,
                }));
            }
        }
    }
